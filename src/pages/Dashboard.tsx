...
import { Clock, Compass, Map, CalendarCheck, Plus } from "lucide-react";
  ...
  return (
    <div className="container mx-auto py-8 px-4">
      ...
      <div className="flex justify-between items-center mb-6">
        <Tabs defaultValue="upcoming" className="w-[400px]" onValueChange={setActiveTab}>
          <TabsList>
              ...
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center text-goginie-gray text-sm">
                    ...
                    <span>{formatDate(trip.dateStart)} - {formatDate(trip.dateEnd)}</span>
                  </div>
                  <div className="flex items-center text-goginie-gray text-sm">
                    <Clock className="h-4 w-4 mr-1" />
                    <span>{Math.round((new Date(trip.dateEnd).getTime() - new Date(trip.dateStart).getTime()) / (1000 * 60 * 60 * 24))} days</span>
                  </div>
                </div>
              </CardContent>
            </Card>
